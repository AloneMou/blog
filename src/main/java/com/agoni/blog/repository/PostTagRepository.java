...
public interface PostTagRepository extends BaseRepository<PostTag, Integer> {
     ...
     */
    @Query("select new com.agoni.blog.model.projection.TagPostPostCountProjection(count(pt.postId), pt.tagId) from PostTag pt where pt.tagId in ?1 group by pt.tagId")
    @NonNull
     ...
     * @return a list of tag post count projection
     */
    @Query("select new com.agoni.blog.model.projection.TagPostPostCountProjection(count(pt.postId), pt.tagId) from PostTag pt group by pt.tagId")
    @NonNull
